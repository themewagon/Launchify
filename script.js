...
const createCard = (card) => `
        <div class="p-4 rounded-lg mx-4 shadow hover:shadow-lg transition-all duration-200 w-72 shrink-0">
            <div class="flex gap-2">
                ...
                <div class="flex flex-col">
                    <div class="flex items-center gap-1">
                        <p>${card.name}</p>
                        <svg class="mt-0.5 text-green-500" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <p class="text-sm pt-4 text-gray-800">
                Launchify helped us ship our product in days instead of weeks. The experience feels effortless.
            </p>
        </div>
    `;
...
function updatePricing() {
    if (isYearly) {
        ...
        btnMonthly.classList.remove('text-green-500');
        btnMonthly.classList.add('text-gray-500', 'hover:text-gray-900');

        btnYearly.classList.remove('text-gray-500', 'hover:text-gray-900');
        btnYearly.classList.add('text-green-500');

        // Update prices
        priceFree.textContent = `$${prices.free}`;
        pricePro.textContent = `$${prices.pro - Math.round(prices.pro * 0.15)}`;
        priceEnterprise.textContent = `$${prices.enterprise - Math.round(prices.enterprise * 0.15)}`;
    } else {
        // Monthly logic
        ...
        btnMonthly.classList.add('text-green-500');

        btnYearly.classList.remove('text-green-500');
        btnYearly.classList.add('text-gray-500', 'hover:text-gray-900');

        // Reset prices
        priceFree.textContent = `$${prices.free}`;
        pricePro.textContent = `$${prices.pro}`;
        priceEnterprise.textContent = `$${prices.enterprise}`;
    }
}
